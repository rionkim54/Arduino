...
void Serial_write(unsigned char * ptr, int len)
{
  ...
  char y = ptr[1];
  ...
  #if NETWORK_MODEL == MODE_2

    if(0 <= x && y <= 0) {}
    else {
      char * pStr = "   MODE_2 # INVALID VECTOR";
      Serial.write(pStr, strlen(pStr));
      return;
    }
  ...
  #if NETWORK_MODEL == MODE_3

    if(x <= 0 && 0 <= y) {}
    else {
      char * pStr = "   MODE_3 # INVALID VECTOR";
      Serial.write(pStr, strlen(pStr));
      return;
    }
    ...
    #if NETWORK_MODEL == MODE_4

    if(x <= 0 && y <= 0) {}
    else {
      char * pStr = "   MODE_4 # INVALID VECTOR";
      Serial.write(pStr, strlen(pStr));
      return;
    }
